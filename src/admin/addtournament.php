...
<?php
    ...
    if ($date_start_raw == NULL) {
        header("Location: /admin?err=temptyDateStart");
        end();
    }
    ...
    if ($date_end_raw == NULL) {
        header("Location: /admin?err=temptyDateEnd");
        end();
    }
    ...
    if ($tournament_name == "") {
        header("Location: /admin?err=temptyName");
        end();
    }

    $rounds = $_POST['rounds'];
    if ($rounds == NULL) {
        header("Location: /admin?err=temptyRound");
        end();
    }
    if (!is_numeric($rounds) || $rounds < 1 || $rounds > 9) {
        header("Location: /admin?err=tinvalidRound");
        end();
    }
    ...
    if ($stmt->affected_rows == 0) {
        header("Location: /admin?err=tfail");
    }
